...
function joox(query) {
    return new Promise((resolve, reject) => {
        ...
        axios.get('http://api.joox.com/web-fcgi-bin//web_search?lang=en&country=ind&type=0&search_input=' + query + '&pn=1&sin=0&ein=29&_=' + time)
            .then(({ data }) => {
                let result = []
                let hasil = []
                let promoses = []
...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: '@Eva – Anirudh', status: false })
                resolve({ creator: '@KRIZSER – Krishnadas', status: true, data: data.links })
            })
    })
}
